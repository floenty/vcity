...
if (isset($_GET['buygarage-id'])) {
    ...
    if ($countgc > 0) {
        ...
        $money   = $rowgc['money'];
        $gold    = $rowgc['gold'];
		$diamond = $rowgc['diamond'];
        $respect = $rowgc['respect'];
        
        if ($rowu['money'] >= $rowgc['money'] && $rowu['gold'] >= $rowgc['gold'] && $rowu['diamond'] >= $rowgc['diamond'] && $rowu['level'] >= $rowgc['min_level'] && $owned == 'No') {
            
            $garage_upgrade = mysqli_query($connect, "UPDATE `players` SET money=money-'$money', gold=gold-'$gold', diamond=diamond-'$diamond', respect=respect+'$respect', garage_id = $buygarage_id WHERE id='$player_id'");
...
<?php
    ...
    } else if ($rowu['money'] < $rowgu['money'] || $rowu['gold'] < $rowgu['gold'] || $rowu['diamond'] < $rowgu['diamond'] || $rowu['level'] < $rowgu['min_level'] || $rowgu['vip'] == 'Yes' && $rowu['role'] != 'VIP') {
