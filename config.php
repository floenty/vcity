...
$user     = "vcity"; // Database Username
$password = "vcity"; // Database's user Password
$database = "vcity"; // Database Name
...
if (isset($_SESSION['username'])) {
    ...
    if ($count > 0) {
        ...
        if ($playerrespect > $bminrespect OR $playerrespect == $bminrespect) {
            $update = mysqli_query($connect, "UPDATE `players` SET level='$blevel', energy='100', money=money+'1000', gold=gold+'2', diamond=diamond+'0' WHERE username='$uname'");
        }
		...
		if ($prow['diamond'] < 0) {
            $player_update = mysqli_query($connect, "UPDATE `players` SET diamond=0 WHERE username='$uname'");
        }
