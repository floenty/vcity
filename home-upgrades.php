...
if (isset($_GET['buyhome-id'])) {
    ...
    if ($counthc > 0) {
        ...
        if ($rowu['money'] >= $rowhc['money'] && $rowu['gold'] >= $rowhc['gold'] && $rowu['diamond'] >= $rowhc['diamond'] && $rowu['level'] >= $rowhc['min_level'] && $owned == 'No') {
            ...
            $home_upgrade = mysqli_query($connect, "UPDATE `players` SET money=money-'$money', gold=gold-'$gold', diamond=diamond-'$diamond', respect=respect+'$respect', home_id = $buyhome_id WHERE id='$player_id'");
