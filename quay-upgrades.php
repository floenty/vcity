...
if (isset($_GET['buyquay-id'])) {
    ...
    if ($countqc > 0) {
        ...
        $money   = $rowqc['money'];
        $gold    = $rowqc['gold'];
		$diamond = $rowqc['diamond'];
        $respect = $rowqc['respect'];
        
        if ($rowu['money'] >= $rowqc['money'] && $rowu['gold'] >= $rowqc['gold'] && $rowu['diamond'] >= $rowqc['diamond'] && $rowu['level'] >= $rowqc['min_level'] && $owned == 'No') {
            
            $quay_upgrade = mysqli_query($connect, "UPDATE `players` SET money=money-'$money', gold=gold-'$gold', diamond=diamond-'$diamond', respect=respect+'$respect', quay_id = $buyquay_id WHERE id='$player_id'");
...
<?php
    ...
    } else if ($rowu['money'] < $rowqu['money'] || $rowu['gold'] < $rowqu['gold'] || $rowu['diamond'] < $rowqu['diamond'] || $rowu['level'] < $rowqu['min_level'] || $rowqu['vip'] == 'Yes' && $rowu['role'] != 'VIP') {
