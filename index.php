...
if (isset($_POST['register'])) {
    ...
    if (mysqli_num_rows($sql) > 0) {
        ...
        if (mysqli_num_rows($sql2) > 0) {
            ...
            $gold     = $row['startgold'];
			$diamond  = $row['startdiamond'];
            $themeid  = $rowtd['id'];
            $langcode = $rowld['langcode'];
            
            $insert = mysqli_query($connect, "INSERT INTO `players` (username, password, email, money, gold, diamond, theme, language) VALUES ('$username', '$password', '$email', '$money', '$gold', '$diamond', '$themeid', '$langcode')");
