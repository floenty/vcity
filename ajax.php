...
<div id="chat">
<?php
    $gtday   = date_default_timezone_set("Asia/Jakarta");
    $gtmonth = date_default_timezone_set("Asia/Jakarta");
    $gtyear  = date_default_timezone_set("Asia/Jakarta");
    
    $gthour   = date_default_timezone_set("Asia/Jakarta");
    $gtminute = date_default_timezone_set("Asia/Jakarta");
...
<?php
    ...
    if (isset($_POST['prize'])) {
        ...
        if ($countcpc > 0) {
			...
			if ($prize == "Diamond") {
                $player_update = mysqli_query($connect, "UPDATE `players` SET diamond=diamond+'$value', spins=spins-1 WHERE id='$player_id'");
            }
            if ($prize == "Respect") {
