...
if (isset($_GET['buy-id'])) {
    ...
    if ($countis > 0) {
        ...
        $money      = $rowis['money'];
        $gold       = $rowis['gold'];
		$diamond    = $rowis['diamond'];
        $respect    = $rowis['respect'];
        $bonustype  = $rowis['bonustype'];
        $bonusvalue = $rowis['bonusvalue'];
        
        if ($rowu['money'] >= $rowis['money'] && $rowu['gold'] >= $rowis['gold'] && $rowu['diamond'] >= $rowis['diamond'] && $rowu['level'] >= $rowis['min_level'] && $owned == 'No') {
            
            if ($bonustype == 'power') {
                $item_pay = mysqli_query($connect, "UPDATE `players` SET money=money-'$money', gold=gold-'$gold', diamond=diamond-'$diamond', respect=respect+'$respect', power=power+'$bonusvalue' WHERE id='$player_id'");
            }
            if ($bonustype == 'agility') {
                $item_pay = mysqli_query($connect, "UPDATE `players` SET money=money-'$money', gold=gold-'$gold', diamond=diamond-'$diamond', respect=respect+'$respect', agility=agility+'$bonusvalue' WHERE id='$player_id'");
            }
            if ($bonustype == 'endurance') {
                $item_pay = mysqli_query($connect, "UPDATE `players` SET money=money-'$money', gold=gold-'$gold', diamond=diamond-'$diamond', respect=respect+'$respect', endurance=endurance+'$bonusvalue' WHERE id='$player_id'");
            }
            if ($bonustype == 'intelligence') {
                $item_pay = mysqli_query($connect, "UPDATE `players` SET money=money-'$money', gold=gold-'$gold', diamond=diamond-'$diamond', respect=respect+'$respect', intelligence=intelligence+'$bonusvalue' WHERE id='$player_id'");
            }
            if ($bonustype == 'energy') {
                $item_pay = mysqli_query($connect, "UPDATE `players` SET money=money-'$money', gold=gold-'$gold', diamond=diamond-'$diamond', respect=respect+'$respect', energy=energy+'$bonusvalue' WHERE id='$player_id'");
            }
            if ($bonustype == 'health') {
                $item_pay = mysqli_query($connect, "UPDATE `players` SET money=money-'$money', gold=gold-'$gold', diamond=diamond-'$diamond', respect=respect+'$respect', health=health+'$bonusvalue' WHERE id='$player_id'");
            }
                    ...
                    <div class="modal-body">
                        <center>
                            ...
                            <div class="row">
								...
								<div class="col-md-4">
                                    <img src="images/icons/diamond.png" width="25%"><br />
                                    <h4>' . lang_key("diamond") . ': <br /><hr /><span class="label label-default">- ' . $diamond . '</span></h4>
                                </div>
                                <div class="col-md-4">
...
if (isset($_GET['sell-id'])) {
    ...
    if ($countspi > 0) {
		...
		$diamond_back   = $rowifsc['diamond'] / 2;
        ...
        if ($bonustypeg == 'power') {
            $values_back = mysqli_query($connect, "UPDATE `players` SET money=money+'$money_back', gold=gold+'$gold_back', diamond=diamond+'$diamond_back', power=power-'$bonusvalueg' WHERE id='$player_id'");
        }
        if ($bonustypeg == 'agility') {
            $values_back = mysqli_query($connect, "UPDATE `players` SET money=money+'$money_back', gold=gold+'$gold_back', diamond=diamond+'$diamond_back', agility=agility-'$bonusvalueg' WHERE id='$player_id'");
        }
        if ($bonustypeg == 'endurance') {
            $values_back = mysqli_query($connect, "UPDATE `players` SET money=money+'$money_back', gold=gold+'$gold_back', diamond=diamond+'$diamond_back', endurance=endurance-'$bonusvalueg' WHERE id='$player_id'");
        }
        if ($bonustypeg == 'intelligence') {
            $values_back = mysqli_query($connect, "UPDATE `players` SET money=money+'$money_back', gold=gold+'$gold_back', diamond=diamond+'$diamond_back', intelligence=intelligence-'$bonusvalueg' WHERE id='$player_id'");
        }
        if ($bonustypeg == 'energy') {
            $values_back = mysqli_query($connect, "UPDATE `players` SET money=money+'$money_back', gold=gold+'$gold_back', diamond=diamond+'$diamond_back', energy=energy-'$bonusvalueg' WHERE id='$player_id'");
        }
        if ($bonustypeg == 'health') {
            $values_back = mysqli_query($connect, "UPDATE `players` SET money=money+'$money_back', gold=gold+'$gold_back', diamond=diamond+'$diamond_back', health=health-'$bonusvalueg' WHERE id='$player_id'");
        }
                    ...
                    <div class="modal-body">
                        <center>
                            ...
                            <div class="row">
								...
								<div class="col-md-6">
                                    <img src="images/icons/diamond.png" width="20%"><br />
                                    <h4>' . lang_key("diamond") . ': <br /><hr /><span class="label label-default">+ ' . $diamond_back . '</span></h4>
                                </div>
                            </div><br /><br />
                            <button type="button" class="btn btn-success btn-md btn-block" data-dismiss="modal" aria-hidden="true"><i class="fab fa-get-pocket"></i> ' . lang_key("okay") . '</button>
                        </center>
...
?></li>
        ...
        echo $rowi['diamond'];
?></span><i class="fas fa-gem"></i> <?php
        echo lang_key("diamond");
?></li>
                        <li class="list-group-item"><span class="badge badge-info"><?php
        echo $rowi['min_level'];
...
<?php
        ...
        } else if ($rowu['money'] < $rowi['money'] || $rowu['gold'] < $rowi['gold'] || $rowu['diamond'] < $rowi['diamond'] || $rowu['level'] < $rowi['min_level'] || $rowi['vip'] == 'Yes' && $rowu['role'] != 'VIP') {
            if ($rowi['bonustype'] == 'health' || $rowi['bonustype'] == 'energy') {
