...
if (isset($_POST['add'])) {
    ...
    $query = mysqli_query($connect, "INSERT INTO `properties` (property, image, money, gold, diamond, respect, income, format, time, min_level, vip) VALUES ('$property', '$image', '$money', '$gold', '$diamond', '$respect', '$income', '$format', '$time', '$minlevel', '$vip')");
}
?>
...
if (isset($_GET['edit-id'])) {
    ...
    if (isset($_POST['edit'])) {
        ...
        $query = mysqli_query($connect, "UPDATE `properties` SET `property`='$property', `image`='$image', `money`='$money', `gold`='$gold', `diamond`='$diamond', `respect`='$respect', `income`='$income', `format`='$format', `time`='$time', `min_level`='$minlevel', `vip`='$vip' WHERE id='$id'");
        echo '<meta http-equiv="refresh" content="0; url=Properties.php">';
