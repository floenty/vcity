...
if (isset($_POST['add'])) {
    ...
    $query = mysqli_query($connect, "INSERT INTO `pets` (name, image, bonustype, bonusvalue, money, gold, diamond, respect, min_level, vip) VALUES ('$name', '$image', '$bonustype', '$bonusvalue', '$money', '$gold', '$diamond', '$respect', '$minlevel', '$vip')");
}
?>
...
if (isset($_GET['edit-id'])) {
    ...
    if (isset($_POST['edit'])) {
        ...
        $query = mysqli_query($connect, "UPDATE `pets` SET `name`='$name', `image`='$image', `bonustype`='$bonustype', `bonusvalue`='$bonusvalue', `money`='$money', `gold`='$gold', `diamond`='$diamond',  `respect`='$respect', `min_level`='$minlevel', `vip`='$vip' WHERE id='$id'");
        echo '<meta http-equiv="refresh" content="0; url=pets.php">';
