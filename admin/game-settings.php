...
if (isset($_POST['save'])) {
	...
	$start_diamond = addslashes(strip_tags($_POST['start-diamond']));
    $paypal_email = addslashes(strip_tags($_POST['paypal-email']));
    
    $content = htmlentities($_POST['content']);
    
    $query = mysqli_query($connect, "UPDATE `settings` SET title='$title', description='$description', keywords='$keywords', startmoney='$start_money', startgold='$start_gold', startdiamond='$start_diamond', paypal_email='$paypal_email', about='$content' WHERE id=1");
...
?>" min="0" required>
											...
											<div class="form-group">
												<label class="col-md-3 control-label" for="inputDefault">Start Diamond:</label>
												<div class="col-md-8">
                                                    <div class="input-group">
                                                    <span class="input-group-addon"><i class="fas fa-gem"></i></span>
													<input type="number" class="form-control" name="start-diamond" value="<?php
echo $row['startdiamond'];
?>" min="0" required>
                                                    </div>
												</div>
											</div>
