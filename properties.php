...
if (isset($_GET['buy-id'])) {
    ...
    if ($countps > 0) {
        ...
        if ($rowu['money'] >= $rowps['money'] && $rowu['gold'] >= $rowps['gold'] && $rowu['diamond'] >= $rowps['diamond'] && $rowu['level'] >= $rowps['min_level'] && $owned == 'No') {
            
            $property_pay = mysqli_query($connect, "UPDATE `players` SET money=money-'$money', gold=gold-'$gold', diamond=diamond-'$diamond', respect=respect+'$respect' WHERE id='$player_id'");
...
<?php
    ...
    } else if ($rowu['money'] < $rowpb['money'] || $rowu['gold'] < $rowpb['gold'] || $rowu['diamond'] < $rowpb['diamond'] || $rowu['level'] < $rowpb['min_level'] || $rowpb['vip'] == 'Yes' && $rowu['role'] != 'VIP') {
