...
if (isset($_GET['buyhangar-id'])) {
    ...
    if ($counthc > 0) {
        ...
        $money   = $rowhc['money'];
        $gold    = $rowhc['gold'];
		$diamond = $rowhc['diamond'];
        $respect = $rowhc['respect'];
        
        if ($rowu['money'] >= $rowhc['money'] && $rowu['gold'] >= $rowhc['gold'] && $rowu['diamond'] >= $rowhc['diamond'] && $rowu['level'] >= $rowhc['min_level'] && $owned == 'No') {
            
            $hangar_upgrade = mysqli_query($connect, "UPDATE `players` SET money=money-'$money', gold=gold-'$gold', diamond=diamond-'$diamond', respect=respect+'$respect', hangar_id = $buyhangar_id WHERE id='$player_id'");
...
<?php
    ...
    } else if ($rowu['money'] < $rowhu['money'] || $rowu['gold'] < $rowhu['gold'] || $rowu['diamond'] < $rowhu['diamond'] || $rowu['level'] < $rowhu['min_level'] || $rowhu['vip'] == 'Yes' && $rowu['role'] != 'VIP') {
